...
def main():
    ...
    for i in tqdm(range(len(pt_data))):
        ...
        try: output_i = json_data_i['response']
        except: output_i = json_data_i['output']

        direct_answer_text = '### Response:' + output_i
        if args.prompt == 'wiz':
            whole_text = instruct_i+'\n\n### Response:'+output_i
        elif args.prompt == 'alpaca':
            try: input_i = json_data_i['input']
            except: input_i = json_data_i['instruction']
            if input_i == '':
        ...
        def get_loss_part_text(tokenizer, text, target_span, max_length, loss_list_):
            ...
            # print(start_token, end_token_real, len(loss_list_))
        ...
        if args.max_length-instruct_i_len > 0:
            ...
            loss_lists_2.append(loss_list_2)
            mean_rate = mean_2/mean_1
            # if mean_rate > 1: 
            #     continue

            mean_rate_list.append((mean_rate,i))
            # mean_list_pair.append((mean_1,mean_2,loss_list_2.tolist()))
    ...
    #     f.write(json.dumps(mean_list_pair))
    
    print(np.mean([mean_rate_list[i][0] for i in range(len(mean_rate_list)) if isinstance(mean_rate_list[i][0],float)]))
