...
# load data
with open('/opt/tiger/LLM/train.csv', newline='') as csvfile:
...
ls = []
[ls.extend(l) for l in labels]
# integer_labels = [int(label) for label in labels]  # 将标签转换为整数
...
def get_embedding(input, model):
    ...
    instruction_embeddings = model_output[0][:, 0]
    print(instruction_embeddings.shape)
